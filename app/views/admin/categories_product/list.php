        ...
        <div class="table-responsive">
          <table class="table table-striped">
            <form action="" method="post">
              ...
              <tbody>
                <?php foreach ($getAllCategoriesProduct as $item) :
                  extract($item);
                ?>
            ...
            <nav aria-label="Page navigation example">
              <div class="d-flex justify-content-between align-items-center">
                <ul class="product__pagination pagination pagination-sm">
                  <li class="page-item <?= $page < 3 ? 'd-none' : false; ?>"><a href="<?= _WEB_ROOT ?>/danh-sach-danh-muc-san-pham/trang-<?= 1; ?>" tabindex="-1"><i class="fa fa-long-arrow-left"></i></a></li>
                  <?php for ($num = 1; $num <= $totalPage; $num++) { ?>
                    <li class="page-item <?= $num == $page ? 'bg-danger' : false; ?>"><a href="<?= _WEB_ROOT ?>/danh-sach-danh-muc-san-pham/trang-<?= $num; ?>"><?= $num; ?></a>
                    </li>
                  <?php } ?>
                  <li class="page-item <?= $page > ($totalPage - 1) ? 'd-none' : false; ?>"><a href="<?= _WEB_ROOT ?>/danh-sach-danh-muc-san-pham/trang-<?= $page + 1; ?>"><i class="fa fa-long-arrow-right"></i></a>
                  </li>
                </ul>
              </div>
            </nav>
